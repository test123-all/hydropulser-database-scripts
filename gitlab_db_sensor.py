...
SENSORS = Namespace(FST["sensors/"])


data = Kraken()
data.g.bind("fst-sensors", SENSORS)
...
# properties
sys_capa = SENSORS[sensor_id + "/SensorCapability"]
...
meas_range = SENSORS[sensor_id + "/MeasurementRange"]
...
sensitivity = SENSORS[sensor_id + "/Sensitivity"]
...
# documentation
img = SENSORS[sensor_id + "/link-to-image.jpg"]
data.g.add((sensor, SDO.subjectOf, img))
data.g.add((sensor, SDO.image, img))

docs = SENSORS[sensor_id + "/link-to-doc-directory"]
data.g.add((sensor, SDO.subjectOf, docs))
data.g.add((sensor, SDO.documentation, docs))

datasheet = SENSORS[sensor_id + "/link-to-doc-directory/datasheet.pdf"]
data.g.add((sensor, SDO.subjectOf, datasheet))
data.g.add((sensor, SDO.documentation, datasheet))

path = "C:/Users/NP/Documents/AIMS/datasheets-mockup/sensors/" + sensor_id + "/"
print(data.g.serialize(destination=path + "rdf.json", format="json-ld", auto_compact=True))
print(data.g.serialize(destination=path + "rdf.ttl", base=SENSORS, format="longturtle"))
print(data.g.serialize(destination=path + "rdf.xml", base=SENSORS, format="xml"))
